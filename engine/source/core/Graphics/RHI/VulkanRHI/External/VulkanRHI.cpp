...
namespace EngineBase {
  ...
  bool VulkanRHI::PrepareVulkan( OS::WindowParameters parameters )
  {
    Window = parameters;
  ...
  bool VulkanRHI::OnWindowSizeChanged() {
  ...
  VkDevice VulkanRHI::GetDevice() const {
    return Vulkan.Device;
  }

  const QueueParameters VulkanRHI::GetGraphicsQueue() const {
    return Vulkan.GraphicsQueue;
  }

  const QueueParameters VulkanRHI::GetPresentQueue() const {
    return Vulkan.PresentQueue;
  }

  const SwapChainParameters& VulkanRHI::GetSwapChain() const {
  ...
  bool VulkanRHI::CreateCommandPool()
  {
      ...
      GetGraphicsQueue().FamilyIndex                              // uint32_t                       queueFamilyIndex
      };

      if (vkCreateCommandPool(GetDevice(), &cmd_pool_create_info, nullptr, &GraphicsCommandPool) != VK_SUCCESS) {
          return false;
      }
      return true;
  }
  ...
  bool VulkanRHI::CreateCommandBuffers()
  {
      ...
      GraphicsCommandBuffers.resize(image_count, VK_NULL_HANDLE);
    ...
    image_count                                           // uint32_t                       bufferCount
      ...
      if (vkAllocateCommandBuffers(GetDevice(), &command_buffer_allocate_info, GraphicsCommandBuffers.data()) != VK_SUCCESS) {
  ...
  bool VulkanRHI::CreateRenderPass()
  {
      ...
      if (vkCreateRenderPass(GetDevice(), &render_pass_create_info, nullptr, &RenderPass) != VK_SUCCESS) {
  ...
  bool VulkanRHI::CreateFrameBuffers()
  {
      ...
      Framebuffers.resize(swap_chain_images.size());
      ...
      for (size_t i = 0; i < swap_chain_images.size(); ++i) {
          ...
          if (vkCreateFramebuffer(GetDevice(), &framebuffer_create_info, nullptr, &Framebuffers[i]) != VK_SUCCESS) {
  ...
  bool VulkanRHI::LoadVulkanLibrary() {
  ...
  bool VulkanRHI::LoadExportedEntryPoints() {
#if defined(VK_USE_PLATFORM_WIN32_KHR)
...
#include "ListOfFunctions.inl"
  ...
  bool VulkanRHI::LoadGlobalLevelEntryPoints() {
...
#include "ListOfFunctions.inl"
  ...
  bool VulkanRHI::LoadInstanceLevelEntryPoints() {
...
#include "ListOfFunctions.inl"
  ...
  bool VulkanRHI::CreatePresentationSurface() {
#if defined(VK_USE_PLATFORM_WIN32_KHR)
  ...
  bool VulkanRHI::CheckPhysicalDeviceProperties( VkPhysicalDevice physical_device, uint32_t &selected_graphics_queue_family_index, uint32_t &selected_present_queue_family_index ) {
    uint32_t extensions_count = 0;
  ...
  bool VulkanRHI::LoadDeviceLevelEntryPoints() {
...
#include "ListOfFunctions.inl"
  ...
  bool VulkanRHI::CreateSwapChainImageViews() {
    for( size_t i = 0; i < Vulkan.SwapChain.Images.size(); ++i ) {
  ...
  bool VulkanRHI::CheckExtensionAvailability( const char *extension_name, const std::vector<VkExtensionProperties> &available_extensions ) {
  ...
  uint32_t VulkanRHI::GetSwapChainNumImages( VkSurfaceCapabilitiesKHR &surface_capabilities ) {
  ...
  VkSurfaceFormatKHR VulkanRHI::GetSwapChainFormat( std::vector<VkSurfaceFormatKHR> &surface_formats ) {
  ...
  VkExtent2D VulkanRHI::GetSwapChainExtent( VkSurfaceCapabilitiesKHR &surface_capabilities ) {
  ...
  VkImageUsageFlags VulkanRHI::GetSwapChainUsageFlags( VkSurfaceCapabilitiesKHR &surface_capabilities ) {
  ...
  VkSurfaceTransformFlagBitsKHR VulkanRHI::GetSwapChainTransform( VkSurfaceCapabilitiesKHR &surface_capabilities ) {
  ...
  VkPresentModeKHR VulkanRHI::GetSwapChainPresentMode( std::vector<VkPresentModeKHR> &present_modes ) {
